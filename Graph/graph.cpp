...
Graph::Graph(std::string provience_addr, std::string distance_addr):_name_to_index(PROVINCE_NUM),_edges(EDGE_NUM){
...
int Graph::get_distance(std::string city_x, std::string city_y, Vector<std::string>* route) {
	...
	int dis_matrix[PROVINCE_NUM][PROVINCE_NUM];
	for (int i = 0; i < PROVINCE_NUM; i++) {
		for (int j = 0; j < PROVINCE_NUM; j++) {
	...
	if (x == -1 || y == -1)return -1;
	dis_matrix[x][x] = 0;
	int last_city[PROVINCE_NUM];
	for (int i = 0; i < PROVINCE_NUM - 1; i++) {
